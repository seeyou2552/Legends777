...
public class UI_GameScene : MonoBehaviour
{
    ...
    public void SetStatus(string name, float status)
    {
        ...
        {
            potionStatusText.text = "Ï≤¥Î†•??" + status.ToString() + " ÎßåÌÅº ?åÎ≥µ?òÏóà?µÎãà??";
        }
        else if (name.StartsWith("Power_Potion"))
        {
            potionStatusText.text = "Í≥µÍ≤©?•Ïù¥ " + status.ToString() + " ÎßåÌÅº ?ÅÏäπ?òÏ??µÎãà??";
        }
        else if (name.StartsWith("AttackSpeed_Potion"))
        {
            potionStatusText.text = "Í≥µÍ≤© ?çÎèÑÍ∞Ä " + status.ToString() + " ÎßåÌÅº ?ÅÏäπ?òÏ??µÎãà??";
        }
