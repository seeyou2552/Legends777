...
public class BossSkillManager : MonoBehaviour
{
    ...
    public Transform firePoint; // Î≥¥Ïä§ ?ÑÏπò Í∞Ä?∏Ïò§Í∏?
    ...
    private List<GameObject> activeSkillObjects = new List<GameObject>(); // ?∞Î°ú ?ùÏÑ±?òÎäî ?§Î∏å?ùÌä∏Î•?Î≥¥Ïä§Í∞Ä Ï£ΩÏùÑ????†ú?òÍ∏∞ ?ÑÌï¥ ?Ä?•Ìï¥?êÎäî Î¶¨Ïä§??
    public List<GameObject> ActiveSkillObjects => activeSkillObjects; // ?∏Ïä§?ôÌÑ∞ Ï∞ΩÏóê???àÎ≥¥?¥Í≥† ?§ÌÅ¨Î¶ΩÌä∏?êÏÑú Ï∞∏Ï°∞?†Ïàò?àÍ≤å

    public string[] sHowEffect = { "Ï≤¥Î†• x 2", "Í≥µÍ≤©??x 2", "?çÎèÑ x 2", "Í≥µÏÜç x 2", "Ï≤¥Î†• / 2", "Í≥µÍ≤©??/ 2", "?çÎèÑ / 2", "Í≥µÏÜç / 2" };
    int getStr;
    ...
    private void Start()
    {
        ...
        SkillsForStage();
        gameManager.OnStageUpdated += SkillsForStage; // ?§ÌÖå?¥Ï? ?¥Î≤§???¨Ïö©
    ...
    private void SkillsForStage() // ?¨Ïö©???§ÌÇ¨?±Î°ù?®Ïàò
    {
    ...
    void ShootFireball(Vector2 direction)
    {
        ...
        // ?§Î∏å?ùÌä∏ ?Ä?êÏÑú ?åÏù¥?¥Î≥º Í∫ºÎÇ¥Í∏?
        GameObject fireball = BossObjectPoolManager.Instance.GetFromPool("Fireball", spawnPos, Quaternion.identity, this.transform);
        if (fireball == null) // ?åÍ¥¥???§Î∏å?ùÌä∏??Í≤ΩÏö∞
        {
            return; // ?¥Îãπ ?§Î∏å?ùÌä∏?????¥ÏÉÅ ?¨Ïö©?????ÜÏúºÎØÄÎ°?Ï¢ÖÎ£å
        }

        // Ï∂©Îèå Ï≤òÎ¶¨ - Î≥¥Ïä§?Ä ?åÏù¥?¥Î≥º??Ï∂©Îèå?òÏ? ?äÎèÑÎ°??§Ï†ï
        ...
        // ?åÏù¥?¥Î≥º???§ÌÅ¨Î¶ΩÌä∏ ?∞Í≤∞ Î∞?Î∞©Ìñ•, ?çÎèÑ ?§Ï†ï
    ...
    void MakeFireBall()
    {
        ...
        {
            attackCount = 1;
            int bulletCount = 8; // Î∞úÏÇ¨???åÏù¥?¥Î≥º??Í∞úÏàò
            float angleStep = 360f / bulletCount; //Ï¥?360?ÑÎ? bulletCountÎ°??òÎàÑ?¥ÏÑú 45??Í∞ÑÍ≤©?ºÎ°ú Î∞úÏÇ¨

            for (int i = 0; i < bulletCount; i++)
            {
                float angle = i * angleStep * Mathf.Deg2Rad; //?ºÎîî??Í∞ÅÎèÑÎ°?Î≥Ä??
                Vector2 direction = new Vector2(Mathf.Cos(angle), Mathf.Sin(angle)); //Cos??xÏ∂?Î∞©Ìñ•, Sin?Ä yÏ∂?Î∞©Ìñ• ?¥Í±∏Î°?unit circle?ÅÏùò Î∞©Ìñ•??Í≥ÑÏÇ∞
                ShootFireball(direction);
            }
        }
    }

    //?åÎ†à?¥Ïñ¥Í∞Ä Ï°¥Ïû¨?òÎäîÏßÄ Ï≤¥ÌÅ¨?òÍ≥† ?§ÌÇ¨?¨Ïö© ?†Î¨¥ ?êÎã®
